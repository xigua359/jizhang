...
function renderRecords() {
  ...
  const matchedRecords = records.filter(r => (!query || `${r.note}${r.category}${r.account}`.toLowerCase().includes(query)) && (filter === '全部' || r.type === filter || r.category === filter));
...
function renderBudget() {
  ...
  document.getElementById('view-budget').innerHTML = `${heading('MONTHLY PLAN', '预算', '', '<button class="primary-button" data-action="open-budget-editor">＋ 新建预算</button>')}
    <div class="budget-layout"><div class="page-card"><div class="panel-header"><div><h2 class="panel-title">分类预算</h2><p class="panel-subtitle">${monthLabel(state.selectedMonth)} · 已支出 ${money(spent)}</p></div><span class="summary-icon">◴</span></div><div class="budget-list">${budgetCards || '<div class="empty-state">还没有设置预算，点击上方按钮添加一个吧。</div>'}</div></div><div class="budget-hero"><p class="eyebrow">BUDGET HEALTH</p><h2 class="panel-title">本月预算状态</h2><div class="budget-ring" style="background:conic-gradient(var(--blue) 0 ${Math.min(100, percent)}%, #dfe7ff ${Math.min(100, percent)}% 100%)"><div class="budget-ring-content"><strong>${percent}%</strong><small>预算使用率</small></div></div><div class="budget-stat"><span>总预算</span><strong>${money(totalBudget)}</strong></div><div class="budget-stat"><span>已支出</span><strong>${money(spent)}</strong></div><div class="budget-stat"><span>剩余可用</span><strong class="positive">${money(totalBudget - spent)}</strong></div></div></div>`;
}
...
document.getElementById('transactionForm').addEventListener('submit', event => { event.preventDefault(); const form = event.currentTarget; const amount = Number(form.elements.amount.value); if (!amount || amount <= 0) return showToast('请输入正确的金额'); const category = form.elements.category.value; const accountName = form.elements.account.value; const record = { id: `r-${Date.now()}`, type: transactionType, amount, category, account: accountName, date: form.elements.date.value, note: form.elements.note.value.trim() || category }; state.records.push(record); const account = state.accounts.find(a => a.name === accountName); if (account && transactionType !== 'transfer') account.balance += transactionType === 'income' ? amount : -amount; saveState(); closeModals(); renderAll(); showToast('账目已保存'); });
